#This script loads the trained model and classifies tweets as safety related tweets or not. (Has no NLTK Dependecy)
...
input_data_file = "/home/gauravbg/SBU-fall-17/Big_Data/Project/Test_Data/test_out_sample.txt"
...
def splitIntoCols(line):
    ...
    date = cols[3]
    loc = cols[4]
    return id, text, username, date, loc
...
if __name__ == "__main__":
    ...
    rdd = rdd.filter(lambda x: len(x) == 5)
    df = sqlContext.createDataFrame(rdd, ["Id", "Original_Text", "Username", "Date", "Location"])
    ...
    dataset = wanted_words_df.withColumnRenamed("Wanted", "text")
    ...
    result.select("Id", "Original_Text", "Date", "Location", "prediction").write.format('com.databricks.spark.csv').save(output_data_file)
