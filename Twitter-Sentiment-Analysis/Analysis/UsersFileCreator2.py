...
USER_DATA_FILE = "users_20-40.csv"
STREAM_STATS_FILE = "stream_stats_2.txt"
...
if __name__ == "__main__":
    ...
    fn = os.path.join(current_file, "temp_user_ids")
    with open(fn, 'r') as file:
        lines = file.readlines()
        for line in lines[20000:40000]:
            u_id = u_name = u_loc = u_tz = is_usa = u_city = u_county = u_state = u_country = NO_DATA_STRING
            total_new_users += 1

            try:
                user_obj = api.get_user(line.strip())
                u_id, u_name, u_loc, u_tz = getUserDetails(user_obj)
                is_usa, u_city, u_county, u_state, u_country = getProcessedLocation(u_loc, u_tz)
                user_dict[u_id] = [u_id, u_name, u_loc, u_tz, is_usa, u_city, u_county, u_state, u_country]
                if is_usa == "YES":
                    us_users_count += 1
                    if u_city == NO_DATA_STRING and u_county == NO_DATA_STRING and u_state == NO_DATA_STRING:
                        us_users_without_loc += 1
            except KeyError as keyError:
                user_obj_not_found_count += 1
            except RateLimitError as limitError:
                print(len(user_dict))
            except TweepError as twError:
                pass
    ...
    with open(stream_stats_file, 'w') as st_file:
        ...
        st_file.write("Total New Users = " + str(total_new_users) + "\n")
