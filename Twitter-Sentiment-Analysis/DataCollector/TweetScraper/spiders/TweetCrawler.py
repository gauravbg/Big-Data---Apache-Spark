    ...
    def parse_tweet_item(self, items):
        for item in items:
            try:
                ...
                twt = item.xpath('.//div[@class="js-tweet-text-container"]/p')[0]
                [twt.remove(i) for i in twt.xpath('./a[@class="twitter-timeline-link"]')]
                tweet['text'] = ' '.join(twt.xpath('.//text()').extract()).replace(' # ', '#').replace(' @ ', '@')
